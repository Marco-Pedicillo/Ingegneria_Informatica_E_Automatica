...
int write_to_pipe(int fd, const void *data, size_t data_len) {
    ...
    while(written_bytes < data_len){
        ...
        if(ret == -1) handle_error("errore scrittura");
        written_bytes += ret;
    }
...
void reader(int reader_id, sem_t* read_mutex) {
    ...
    ret = close(pipefd[1]);
    if(ret) handle_error("errore chiusura pipe scrittura");
    ...
    ret = close(pipefd[0]);
    if(ret) handle_error("errore chiusura pipe lettura");
...
int main(int argc, char* argv[]) {
    ...
    ret = pipe(pipefd);
    if(ret) handle_error("errore creazione pipe");
