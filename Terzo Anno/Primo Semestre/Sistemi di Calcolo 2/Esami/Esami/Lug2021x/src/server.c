...
int main(int argc, char *argv[])
{
    ...
    if(listening_socket_desc == -1) handle_error("errore socket");
     ...
     */
	 ...
	 ret = bind(listening_socket_desc, (struct sockaddr*)&server_addr, (socklen_t)sockaddr_len);
     if(ret) handle_error("errore bind");
     ret = listen(listening_socket_desc, MAX_CONN_QUEUE);
     if(ret) handle_error("errore listen");
    ...
    while (1) {
        ...
        if(close(client_desc)) handle_error("errore close");
    ...
    if(close(listening_socket_desc)) handle_error("errore close");
