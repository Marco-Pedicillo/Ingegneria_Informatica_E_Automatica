...
void create_fifo(char* name, mode_t mode) {
    ...
    int ret;
    if(unlink(name)) ERROR_HELPER(ret, "errore unlink");
    ret = mkfifo(name, mode);
    if(ret) ERROR_HELPER(ret, "errore mkfifo");
...
int main(int args, char* argv[]) {    
    ...
    if(first_fifo == -1) ERROR_HELPER(first_fifo, "errore open f1");

    second_fifo = open(SECOND_FIFO_NAME, O_RDONLY);
    if(second_fifo == -1) ERROR_HELPER(second_fifo, "errore open f2");
    ...
    int ret;
    ret = close(first_fifo);
    if(ret) ERROR_HELPER(ret, "errore close");
