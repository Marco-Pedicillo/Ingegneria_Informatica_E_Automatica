...
void openMemory() {
    ...
    if(fd_shm < 0) handle_error("errore shm_open");
...
void request(){
    ...
    while(1){
        ...
        if(sem_wait(&myshm_ptr->sem_filled)) handle_error("errore wait");
        if(sem_wait(&myshm_ptr->sem_mem)) handle_error("errore wait");
        ...
        if(sem_post(&myshm_ptr->sem_mem)) handle_error("errore post");
        if(sem_post(&myshm_ptr->sem_empty)) handle_error("errore post");
...
void closeSemaphores() {
    ...
    if(sem_destroy(&myshm_ptr->sem_filled)) handle_error("errore chiusura sem");
    if(sem_destroy(&myshm_ptr->sem_empty)) handle_error("errore chiusura sem");
    if(sem_destroy(&myshm_ptr->sem_mem)) handle_error("errore chiusura sem");
