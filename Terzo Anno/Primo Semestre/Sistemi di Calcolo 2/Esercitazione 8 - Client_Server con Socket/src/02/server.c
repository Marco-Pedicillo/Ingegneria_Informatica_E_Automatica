...
void* connection_handler(int socket_desc) {
   ...
   // int sockaddr_len = sizeof(client_addr);
    ...
    while (1) {
        ...
        recv_bytes = 0;
        do {
            ret = recvfrom(socket_desc, buf, buf_len, 0, (struct sockaddr*)&client_addr, sizeof(struct sockaddr_in));
        ...
        while ( bytes_sent < recv_bytes) {
            ret = sendto(socket_desc, buf, recv_bytes, 0, (struct sockaddr*) &client_addr, sizeof(struct sockaddr_in));
            if (ret == -1 && errno == EINTR) continue;
