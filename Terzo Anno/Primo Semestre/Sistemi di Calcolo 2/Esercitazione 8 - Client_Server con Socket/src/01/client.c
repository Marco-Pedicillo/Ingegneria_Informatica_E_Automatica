...
int main(int argc, char* argv[]) {
    ...
    if(socket_desc < 0) handle_error("errore socket create");
    ...
    if(ret < 0) handle_error("errore connect");
...
//        buf[--msg_len] = '\0'; // remove '\n' from the end of the message
        ...
        if(bytes_sent == quit_command_len && !memcmp(buf, quit_command, quit_command_len)){
            if(DEBUG) fprintf(stderr, "comando QUIT\n");
            break;
        }
