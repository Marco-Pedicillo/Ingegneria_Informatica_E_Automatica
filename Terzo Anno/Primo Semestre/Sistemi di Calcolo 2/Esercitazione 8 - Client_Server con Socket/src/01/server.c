...
void* connection_handler(int socket_desc) {
    ...
    while(bytes_sent < msg_len){
        ...
        if(ret == -1) handle_error("errore write socket");
        bytes_sent += ret;
    }
    ...
    while (1) {
        ...
        if(recv_bytes == quit_command_len && !memcmp(buf, quit_command, quit_command_len)){
            if(DEBUG) fprintf(stderr, "comando  QUIT ricevuto\n");
            break;
        }
...
int main(int argc, char* argv[]) {
    ...
    if(socket_desc < 0) handle_error("errore create socket"); 
    ...
    if(ret < 0) handle_error("errore bind");
    ...
    if(ret < 0) handle_error("errore listen");
    ...
    while (1) {
        ...
        if(client_desc == -1) handle_error("errore accept");
