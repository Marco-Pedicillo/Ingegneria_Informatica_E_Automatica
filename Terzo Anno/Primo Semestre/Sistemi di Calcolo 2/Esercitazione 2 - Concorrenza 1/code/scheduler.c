...
void* client(void* arg_ptr) {
    ...
    if(ret){
        fprintf(stderr, "non posso acquisire il semaforo dal thread %d: %s\n", args -> ID, strerror(errno));
        exit(1);
    }
    ...
    if(ret){
        fprintf(stderr, "non posso rilasciare il semaforo dal thread %d: %s\n", args -> ID, strerror(errno));
        exit(1);
    }
...
int main(int argc, char* argv[]) {
    ...
    if(ret){
        fprintf(stderr, "non posso creare il semaforo: %s\n", strerror(errno));
        exit(1);
    }
