...
void* thread_work(void *arg) {
	...
	for (i = 0; i < m; i++){
        if(sem_wait(&sem) != 0){
            fprintf(stderr, "non posso acquisire il semaforo, errore %d\n", errno);
            return NULL;
        }
        ...
        if(sem_post(&sem) != 0){
            fprintf(stderr, "non posso rilasciare il semaforo, errore %d\n", errno);
            return NULL;
        }
    }
    return NULL;
...
int main(int argc, char **argv)
{
    ...
    if(sem_init(&sem, 0, 1) != 0){
        fprintf(stderr, "non posso inizializzare il semaforo, errore %d\n", errno);
        exit(EXIT_FAILURE);
    }
